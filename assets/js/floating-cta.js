...
(function(){
  ...
  var html = css +
    '<div id="snt-fab" class="snt-fab" role="region" aria-label="Quick contact">' +
      ...
      '</button>' +
    '</div>' +
    '<div id="snt-wechat-modal" class="snt-wcm" role="dialog" aria-label="WeChatで連絡" onclick="if(event.target===this)this.classList.remove(\'is-open\');">' +
      '<div class="snt-wcm__box">' +
        '<button class="snt-wcm__close" onclick="document.getElementById(\'snt-wechat-modal\').classList.remove(\'is-open\');" aria-label="閉じる">×</button>' +
        '<h3 class="snt-wcm__title">WeChatで連絡</h3>' +
        '<p class="snt-wcm__desc">QRコードをスキャンするか、WeChat IDを検索:</p>' +
